...
def evaluate_no_label(dataloader):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for idx, (text, offsets) in enumerate(dataloader):
            predicted_label = model(text, offsets)
            predicted_label = predicted_label.flatten()
            for _ in predicted_label:
                ret.append(torch.round(_))
        return ret
...
if __name__ == '__main__':
    ...
    no_label_train_dataset = CustomDataSet(no_label_data)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print('-' * 59)
    print("*" * 59)
    print("开始预测no-label数据")
    print("*" * 59)
    # evaluate the no-label datasets
    evaluate_dataloader = DataLoader(no_label_train_dataset, shuffle=False, collate_fn=no_label_collate_batch)
    label__ = evaluate_no_label(evaluate_dataloader)
